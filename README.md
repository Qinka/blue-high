...
# 添加 ARM Cortex-M 目标（thumbv7m-none-eabi 用于 STM32F103）
...
**注意**：本项目包含 `rust-toolchain.toml` 文件，会自动安装所需的目标架构。
...
probe-rs 会通过 RTT (Real-Time Transfer) 显示实时日志输出：
...
📺 [OLED] I2C2 @ PB10/PB11 初始化完成
📡 [E22] SPI1 @ PA5/PA6/PA7 初始化完成
╔══════════════════════════════════╗
║      E22-400M30S LoRa 配置       ║
║ 频率: 433 MHz
║ 功率: 30 dBm
...
**重要提示**：
- RTT 日志需要通过 SWD 调试器连接（SWDIO, SWCLK, GND）
- 使用 ST-Link V2 或兼容调试器
- 确保 probe-rs 正确识别 STM32F103C8 芯片
...
2. 确保 thumbv7m-none-eabi 目标已安装：
   ```bash
   rustup target list --installed | grep thumbv7m-none-eabi
   ```

3. 检查 probe-rs 是否识别芯片：
   ```bash
   probe-rs chip list | grep STM32F103
   ```

4. 尝试手动烧录：
   ```bash
   cargo build --release
   probe-rs run --chip STM32F103C8 target/thumbv7m-none-eabi/release/blue-high
   ```
